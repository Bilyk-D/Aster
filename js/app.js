(() => {
    ...
    if (spollersArray.length > 0) {
      ...
      function hideSpollersBody(spollersBlock) {
        ...
        if (
          ...
          const spollerActiveTitle =
            spollerActiveBlock.querySelector("summary");
          const spollerSpeed = spollersBlock.dataset.spollersSpeed
            ? parseInt(spollersBlock.dataset.spollersSpeed)
            : 500;
  ...
  function initSliders() {
    if (document.querySelector(".creators-module__slider"))
      new swiper_core_Swiper(".creators-module__slider", {
        modules: [Navigation],
        observer: true,
        observeParents: true,
        slidesPerView: 5,
    ...
    if (document.querySelector(".quick-bytes__slider"))
      new swiper_core_Swiper(".quick-bytes__slider", {
      ...
      if (news.length) {
        ...
        news.forEach((newsItem) => {
          ...
          if (!newsItem.image) {
            ...
            templateImageItem = templateImageItem.replace(
              `%image%`,
              newsItem.image
            );
            templateItem = templateItem.replace(
              `%imagePlace%`,
              templateImageItem
            );
          }
  ...
  } else console.log("More news don't loaded");
  const weatherBlock = document.querySelector("#weather");
  async function loadWeather(e) {
    weatherBlock.innerHTML = `\n  <div id="weather" class="body-weather-module__icon">\n  <img src="" alt="">\n</div>`;
    const server =
      "https://api.openweathermap.org/data/2.5/weather?units=metric&q=Wroclaw&appid=522f7ec766b55c89fccbc47a4e7a72c0";
    const response = await fetch(server, {
      method: "GET",
    });
    const responseResult = await response.json();
